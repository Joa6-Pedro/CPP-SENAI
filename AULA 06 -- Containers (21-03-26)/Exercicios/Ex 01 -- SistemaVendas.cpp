...
int main(){
    ...
    std::array<std::string, 7> nomeDias = {
        "Domingo",
        "Segunda-Feira",
        "Terça-Feira",
        "Quarta-Feira",
    ...
    std::string diaMaiorVenda, diaMenorVenda;
    
    for (int i = 0; i < dias.size(); i++) {
        
        //Entrada de valores
        std::cout << "Digite o valor para o dia \" " << nomeDias[i] << "\": ";
        std::cin >> dias[i];
        ...
        if (i == 0) {
            ...
            maior = dias[i];
            diaMaiorVenda = nomeDias[i];
        }else if(dias[i] < menor) {
            menor = dias[i];
            diaMenorVenda = nomeDias[i];
        }
